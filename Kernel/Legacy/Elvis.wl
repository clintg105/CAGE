...
lineThroughPointsEqu[{{x1_, y1_}, {x2_, y2_}}] := 
  y - y1 == (y2 - y1)/(x2 - x1) (x - x1);
lineThroughPointsYFunc[{{x1_, y1_}, {x2_, y2_}}] := 
 Function[{x}, (y2 - y1)/(x2 - x1) (x - x1) + y1]
lineThroughPointsRFunc[{{x1_, y1_}, {x2_, y2_}}] := Module[
  {r1 = Norm[{x1, y1}], \[Theta]1 = atan2[y1, x1], \[Theta]0 = \!\(\*
TagBox[GridBox[{
{"\[Piecewise]", GridBox[{
{
RowBox[{"ArcTan", "[", 
...
Selectable->False,
StripWrapperBoxes->True]\)}, 
  Function[{\[Theta]}, (r1 Sin[\[Theta]1 - \[Theta]0])/
   Sin[\[Theta] - \[Theta]0]]]
...
DF=5;
discretize[x_]:=If[ConeQ[x],Sequence@@Table[x/.\[Lambda]->t,{t,0,1,1/(DF-1)}],x]
...
Subscript[\[Sigma], F]=Function[{v},Evaluate[Simplify[FullSimplify@Evaluate[Max[
{v[[1]],v[[2]]} . {x,y}/.Solve[Eliminate[D[{v[[1]],v[[2]]} . {x,y}-\[Lambda](#[[2]]-#[[1]]&[Subscript[F, bdryCnds]]),{{x,y,\[Lambda]}}]==0,\[Lambda]],{x,y}]
]],(v[[1]]|v[[2]])\[Element]Reals]]];
Subscript[\[Gamma], F^\[Degree]]=Subscript[\[Sigma], F];
Subscript[\!\(\*OverscriptBox[\(\[ScriptCapitalN]\), \(^\)]\), F^\[Degree]]=Function[{\[Zeta]},Evaluate[\!\(
\*SubscriptBox[\(\[Del]\), \({x, y}\)]\(
\(\*SubscriptBox[\(\[Gamma]\), 
SuperscriptBox[\(F\), \(\[Degree]\)]]\)[{x, y}]\)\)/.{x->\[Zeta][[1]],y->\[Zeta][[2]]}]];(* expects pre-scaled vector on bdry(F^\[Degree])*)

Subscript[F^\[Degree], bdryCnds]=Subscript[\[Gamma], (F^\[Degree])][{x,y}]==1;
Subscript[\[Sigma], F^\[Degree]]=Function[{\[Zeta]},Evaluate[Simplify[FullSimplify@Evaluate[Max[
{\[Zeta][[1]],\[Zeta][[2]]} . {x,y}/.Solve[Eliminate[D[{\[Zeta][[1]],\[Zeta][[2]]} . {x,y}-\[Lambda](#[[2]]-#[[1]]&[Subscript[F^\[Degree], bdryCnds]]),{{x,y,\[Lambda]}}]==0,\[Lambda]],{x,y}]
]],(\[Zeta][[1]]|\[Zeta][[2]])\[Element]Reals]]];
Subscript[\[Gamma], F]=Subscript[\[Sigma], F^\[Degree]];
Subscript[\!\(\*OverscriptBox[\(\[ScriptCapitalN]\), \(^\)]\), F]=Function[{v},Evaluate[\!\(
\*SubscriptBox[\(\[Del]\), \({x, y}\)]\(
\(\*SubscriptBox[\(\[Gamma]\), \(F\)]\)[{x, y}]\)\)/.{x->v[[1]],y->v[[2]]}]];(* expects pre-scaled vector on bdry(F)*)

Subscript[RefractedNormals, F]=Function[{\[Zeta]},Evaluate[
(* refreacted zeta must have the same tangent component as incident zeta *)
{\[Zeta][[1]],#}&/@DeleteCases[(Last/@First/@Solve[N@Subscript[(F^\[Degree]), bdryCnds],{y},Reals])/.{x->\[Zeta][[1]]},Undefined]
]];
